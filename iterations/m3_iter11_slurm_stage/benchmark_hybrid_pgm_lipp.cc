...
void benchmark_64_hybrid_pgm_lipp(tli::Benchmark<uint64_t>& benchmark,
  ...
  if (filename.find("0.100000i") != std::string::npos) {
    ...
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 128, 128, 48>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, ExponentialSearch<record>, 256, 256, 64>>();
    return;
  }

  if (filename.find("0.900000i") != std::string::npos) {
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 128, 128, 24>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 256, 256, 32>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, ExponentialSearch<record>, 256, 512, 48>>();
  }
}
