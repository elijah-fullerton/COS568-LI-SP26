...
class HybridPGMLIPP : public Competitor<KeyType, SearchClass> {
 public:
  ...
  uint64_t Build(const std::vector<KeyValue<KeyType>>& data, size_t num_threads) {
    ...
    const uint64_t build_time = util::timing([&] {
      ...
      global_mutable_bloom_.assign(kGlobalBloomBits / 64, 0);
  ...
  size_t EqualityLookup(const KeyType& lookup_key, uint32_t thread_id) const {
    ...
    if (!GlobalBloomMayContain(lookup_key)) {
    ...
    if (published != nullptr && published->has_range &&
        ...
        published->MaybeContains(lookup_key)) {
      const uint64_t overlay_value = LookupPublished(*published, lookup_key);
      if (overlay_value != util::OVERFLOW) {
        return overlay_value;
      }
    }

    if (bucket.mutable_entries.load(std::memory_order_acquire) == 0) {
      return util::OVERFLOW;
    }
    ...
    for (size_t bucket_id = lower_bucket; bucket_id <= upper_bucket; ++bucket_id) {
      ...
      if (published != nullptr && published->has_range &&
          RangesOverlap(lower_key, upper_key, published->min_key,
                        published->max_key)) {
        result += RangeQueryPublished(*published, lower_key, upper_key);
      }
  ...
  void Insert(const KeyValue<KeyType>& data, uint32_t thread_id) {
    ...
    UpdateRange(bucket.active_has_range, bucket.active_min, bucket.active_max,
                data.key);
    SetGlobalBloomBit(data.key);
    bucket.mutable_entries.store(bucket.active_size + bucket.pending_size,
                                 std::memory_order_release);
 ...
 private:
  ...
  static constexpr size_t kGlobalBloomBits = 1u << 24;
  ...
  uint64_t LookupPublished(const PublishedBucketState& state,
                           const KeyType& lookup_key) const {
    if (state.items == nullptr || state.items->empty()) {
      return util::OVERFLOW;
    }
    auto it = std::lower_bound(
        state.items->begin(), state.items->end(), lookup_key,
        [](const typename PublishedBucketState::ValueType& item,
           const KeyType& key) { return item.first < key; });
    if (it != state.items->end() && it->first == lookup_key) {
      return it->second;
    }
    return util::OVERFLOW;
  }

  uint64_t RangeQueryPublished(const PublishedBucketState& state,
    ...
    uint64_t result = 0;
    auto it = std::lower_bound(
        state.items->begin(), state.items->end(), lower_key,
        [](const typename PublishedBucketState::ValueType& item,
           const KeyType& key) { return item.first < key; });
    while (it != state.items->end() && it->first <= upper_key) {
      result += it->second;
      ++it;
    }
    return result;
  ...
  struct BucketState {
    ...
    KeyType pending_max{};
    bool queued{false};
    std::atomic<size_t> mutable_entries{0};
  ...
  void HandoffBucket(size_t bucket_id, BucketState& bucket) {
    ...
    bucket.pending_size = bucket.active_size;
    ...
    bucket.mutable_entries.store(bucket.pending_size, std::memory_order_release);
  ...
  bool GlobalBloomMayContain(const KeyType& key) const {
    const uint64_t hash1 = PublishedBucketState::HashKey(key);
    const uint64_t hash2 =
        PublishedBucketState::RotateLeft(hash1 * 0xD1342543DE82EF95ULL, 11);
    return TestGlobalBloomBit(static_cast<size_t>(hash1) & (kGlobalBloomBits - 1)) &&
           TestGlobalBloomBit(static_cast<size_t>(hash2) & (kGlobalBloomBits - 1));
  }

  void SetGlobalBloomBit(const KeyType& key) {
    const uint64_t hash1 = PublishedBucketState::HashKey(key);
    const uint64_t hash2 =
        PublishedBucketState::RotateLeft(hash1 * 0xD1342543DE82EF95ULL, 11);
    SetGlobalBloomBitAt(static_cast<size_t>(hash1) & (kGlobalBloomBits - 1));
    SetGlobalBloomBitAt(static_cast<size_t>(hash2) & (kGlobalBloomBits - 1));
  }

  bool TestGlobalBloomBit(size_t bit) const {
    const size_t word = bit >> 6;
    const uint64_t mask = uint64_t{1} << (bit & 63);
    return (global_mutable_bloom_[word] & mask) != 0;
  }

  void SetGlobalBloomBitAt(size_t bit) {
    const size_t word = bit >> 6;
    global_mutable_bloom_[word] |= uint64_t{1} << (bit & 63);
  }
  ...
  std::vector<uint64_t> global_mutable_bloom_;
