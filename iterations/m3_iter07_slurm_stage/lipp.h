...
public:
    ...
    double root_model_a() const {
        return root->model.a;
    }

    long double root_model_b() const {
        return root->model.b;
    }

    int root_num_items() const {
        return root->num_items;
    }

    void freeze_existing_nodes() {
        freeze_existing_subtree(root);
    }
    ...
    int locate_buffer_owner(const T& key) const {
        int owner_id = 0;
        Node* node = root;

        while (true) {
            if (node->buffer_owner_id >= 0) {
                owner_id = node->buffer_owner_id;
            }
            ...
            if (BITMAP_GET(node->child_bitmap, pos) == 1) {
                ...
                RT_ASSERT(owner_id >= 0);
                return owner_id;
            }
        }
    }

    bool find_with_buffer_owner(const T& key, P& value, int& owner_id) const {
        owner_id = -1;
        Node* node = root;
...
private:
    ...
    bool has_non_child_slot(const Node* node) const
    {
        for (int i = 0; i < node->num_items; ++i) {
    ...
    void freeze_existing_subtree(Node* node)
    {
        if (node == NULL || node->frozen_existing) {
            return;
        }
        node->fixed = 1;
        node->frozen_existing = 1;
        node->buffer_owner_id = -1;
        for (int i = 0; i < node->num_items; ++i) {
            if (BITMAP_GET(node->child_bitmap, i) == 1) {
                freeze_existing_subtree(node->items[i].comp.child);
            }
        }
    }
    ...
    void assign_buffer_owners_recursive(Node* node, int owner_max_size, int& next_owner_id)
    {
        ...
        if (!node->frozen_existing) {
            return;
        }

        if (node->size <= owner_max_size || !has_child(node)) {
            node->buffer_owner_id = next_owner_id ++;
            return;
        }

        if (has_non_child_slot(node)) {
            node->buffer_owner_id = next_owner_id ++;
        }
