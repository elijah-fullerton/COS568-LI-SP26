...
void benchmark_64_hybrid_pgm_lipp(tli::Benchmark<uint64_t>& benchmark,
  ...
  if (filename.find("0.100000i") != std::string::npos) {
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 64, 512, 48>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 64, 1024, 24>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 64, 1024, 32>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, ExponentialSearch<record>, 64, 2048, 32>>();
    return;
  }
  ...
  if (filename.find("0.900000i") != std::string::npos) {
    ...
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 64, 512, 192>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 64, 1024, 256>>();
    benchmark.template Run<
