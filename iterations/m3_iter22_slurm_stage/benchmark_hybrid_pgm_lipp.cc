...
void benchmark_64_hybrid_pgm_lipp(tli::Benchmark<uint64_t>& benchmark,
  ...
  if (filename.find("0.100000i") != std::string::npos) {
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 64, 64, 8>>();
    benchmark.template Run<
  ...
  if (filename.find("0.900000i") != std::string::npos) {
    ...
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, BranchingBinarySearch<record>, 128, 512, 128>>();
    benchmark.template Run<
        HybridPGMLIPP<uint64_t, ExponentialSearch<record>, 128, 1024, 256>>();
  }
}
