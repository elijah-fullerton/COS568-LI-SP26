...
STAGE_DIR="${STAGE_DIR:-${REPO_ROOT}/iterations/m3_iter11_slurm_stage}"
ARCHIVE_ROOT="${ARCHIVE_ROOT:-${REPO_ROOT}/slurm_runs/m3_iter11}"
SCRATCH_ROOT="${SCRATCH_ROOT:-/scratch/${USER}/m3_iter11_job_${SLURM_JOB_ID}}"
...
mkdir -p "${SCRATCH_ROOT}/artifacts" "${SCRATCH_ROOT}/logs"
...
cleanup() {
  ...
  cp -rf "${SCRATCH_ROOT}/workspace/results_milestone3" \
    "${ARCHIVE_ROOT}/results.${SLURM_JOB_ID}" 2>/dev/null || true
  exit "${rc}"
}
trap cleanup EXIT
...
/usr/bin/time -v \
  timeout 12h \
  env BENCHMARK_PATH=build_scratch/benchmark DATA_DIR=./data RESULTS_DIR=results_milestone3 BENCHMARK_REPEATS=3 \
...
if [[ "${rc}" == "0" ]]; then
  python3 ./scripts/analysis_milestone3.py
fi
