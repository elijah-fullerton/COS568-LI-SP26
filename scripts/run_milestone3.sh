...
RESULTS_DIR="${RESULTS_DIR:-results_milestone3}"
INDEXES=(
    "DynamicPGM"
    "LIPP"
    "HybridPGMLIPP"
...
mkdir -p "${RESULTS_DIR}"
rm -f "${RESULTS_DIR}"/books_100M_public_uint64*_results_table.csv
rm -f "${RESULTS_DIR}"/fb_100M_public_uint64*_results_table.csv
rm -f "${RESULTS_DIR}"/osmc_100M_public_uint64*_results_table.csv
...
echo "Milestone 3 benchmark runs complete."
