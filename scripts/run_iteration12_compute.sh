...
STAGE_DIR="${STAGE_DIR:-${REPO_ROOT}/iterations/m3_iter12_slurm_stage}"
ARCHIVE_ROOT="${ARCHIVE_ROOT:-${REPO_ROOT}/slurm_runs/m3_iter12}"
SCRATCH_ROOT="${SCRATCH_ROOT:-/scratch/${USER}/m3_iter12_job_${SLURM_JOB_ID}}"
...
cp "${STAGE_DIR}/pgm_index_dynamic.hpp" \
  "${SCRATCH_ROOT}/workspace/competitors/PGM-index/include/pgm_index_dynamic.hpp"
...
if [[ "${VERIFY_ONLY}" == "1" ]]; then
  ...
  /usr/bin/time -v \
    timeout 8h \
    env BENCHMARK_PATH=build_scratch/benchmark DATA_DIR=./data \
      RESULTS_DIR="${SCRATCH_ROOT}/results" BENCHMARK_REPEATS=3 \
